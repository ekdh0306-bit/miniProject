...
class MemberService:
    ...
    @staticmethod
    def update_member(user_id, uid, email, password=None):
        conn = Session.get_connection()
        try:
            with conn.cursor() as cursor:
                # 비밀번호가 입력되었을 경우와 아닌경우를 구분해서 sql작성
                if password:
                    sql = "UPDATE members SET uid = %s, email = %s, password = %s WHERE id = %s"
                    cursor.execute(sql, (uid, email, password, user_id ))
                else:
                    sql = "UPDATE members SET uid = %s, email = %s WHERE id = %s"
                    cursor.execute(sql, (uid, email, user_id))
                print(f"받은 user_id: {user_id}")
                print(f"받은 user_id type: {type(user_id)}")
                print(f"실행 SQL: UPDATE members SET uid = {uid}, email = {email} WHERE id = {user_id}")
