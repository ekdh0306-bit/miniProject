...
def find_id():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if row:
                # 💡 페이지 이동 대신 JSON 데이터를 반환합니다.
                return {"success": True, "uid": row['uid']}
            else:
                return {"success": False, "message": "일치하는 계정이 없습니다."}
    finally:
        conn.close()
...
def member_edit():
    try:
        ...
        # 프로필 파일 처리 (파일이 있을 때만 로직 실행)
        file = request.files.get('profile')
        profile_path = None
        if file and file.filename != '':
            # 저장 경로 설정 (리나님의 환경에 맞게 수정)
            profile_path = f"static/uploads/{file.filename}"
            file.save(profile_path)

        conn = Session.get_connection()  #
        try:
            with conn.cursor() as cursor:
                # 2. SQL 문에 bio와 profile_img 추가
                if new_pw:
                    sql = """UPDATE members \
                             SET uid=%s, \
                                 name=%s, \
                                 ...
                                 password=%s, \
                                 bio=%s """
                    params = [new_uid, new_name, new_email, new_pw, bio]
                    if profile_path:
                        sql += ", profile_img=%s "
                    ...
                    # 비밀번호 미입력 시 처리 로직도 위와 동일하게 bio 추가!
                    sql = "UPDATE members SET uid=%s, name=%s, email=%s, bio=%s "
                    params = [new_uid, new_name, new_email, bio]
                    if profile_path:
                        sql += ", profile_img=%s "
                        params.append(profile_path)
                    sql += "WHERE id=%s"
